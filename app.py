...
TOLERANCIA     = 0.01
MAX_NOTAS      = 30   # suporta combinações com muitas notas
TOP_RESULTADOS = 10
...
def _verificar_soma_total(df_sub, alvo):
    """
    Verifica se a soma de todas as notas (usando o melhor valor disponível
    por nota: saldo+ir+iss → saldo+ir → saldo+iss → saldo) = alvo.
    Retorna lista de chaves se bater, None caso contrário.
    """
    chaves = []
    soma   = 0.0
    for idx, row in df_sub.iterrows():
        ops = gerar_opcoes_nota(row)
        # Pegar opção de maior dedução (última = saldo_ir_iss se existir)
        nome, valor = ops[-1]
...
def conciliar(df_cli, valor_alvo, escopo="direto"):
    ...
    if escopo == "cidade":
        ...
        # Atalho: verificar soma total antes de qualquer busca combinatória
...
def _conciliar_cascata_cidade(df_cidade, valor_alvo):
    ...
    for cnpj_val in df_cidade[col_cnpj].unique():
        ...
        # Atalho: soma total do CNPJ = alvo?
    ...
    if "rbase" in df_cidade.columns:
        for rbase_val in df_cidade["rbase"].unique():
            ...
            if df_sub["cnpj_limpo" if "cnpj_limpo" in df_cidade.columns else "cnpj"].nunique() < 2:
                continue
            # Atalho: soma total do grupo = alvo?
            chaves_t, soma_t = _verificar_soma_total(df_sub, valor_alvo)
            if chaves_t:
                meta = {"pre": len(df_sub), "camada": "Soma Total",
    ...
    # Atalho: soma de toda a cidade = alvo?
...
def exibir_combo(combo, i, n_total):
    ...
    # HTML sem f-strings com aspas conflitantes (Python 3.12+)
    ...
    tags_html = "".join(tags)
    html_card = (
        "<div style='border:" + borda + ";border-radius:12px;overflow:hidden;margin-bottom:20px;" + sombra + "'>"
        ...
        if executar:
            if not identificador:
                ...
                if bp == "CIDADE" and resultados:
                    ...
                    if cor == "purple":   css_class = "alert-escopo"
                    elif cor == "yellow": css_class = "alert-multi"
                    elif cor == "red":    css_class = "alert-retencao"
                    else:                 css_class = "alert-multi"
                    if descricao:
                        st.markdown(f"""
                        <div class="{css_class}">
...
def _bb_ler_extrato(file_bytes, file_name):
    """
    Lê o extrato BB preservando TODAS as colunas originais.
    Detecta automaticamente qual linha é o cabeçalho (primeira linha não-vazia).
    Acrescenta colunas internas _AGENCIA_ORIG e _DETALHAMENTO para uso no enriquecimento.
    """
    import io, openpyxl as _opx
    wb = _opx.load_workbook(io.BytesIO(file_bytes))
    ws = wb.active

    # Encontrar linha do cabeçalho (primeira com células não-nulas)
    header_row  = None
    header_vals = []
    ...
    if header_row is None:
        raise ValueError("Não foi possível encontrar o cabeçalho na planilha BB.")

    # Ler dados abaixo do cabeçalho
    rows_data = []
    for i, row in enumerate(ws.iter_rows(values_only=True), 1):
        if i <= header_row:
            continue
    ...
    # Montar DataFrame com nomes originais (tratar duplicatas)
    col_names = []
    seen_cols = {}
    for v in header_vals:
        name = str(v).strip().upper() if v is not None else "COL"
    ...
    df = df[df.apply(lambda r: any(v not in (None, "", "NONE", "NAN")
                                    for v in r.astype(str)), axis=1)].copy()
    df.index = range(len(df))

    # Identificar colunas-chave para o enriquecimento
    # Agência: coluna cujo nome contém "AGENCIA" ou "AG"
    col_ag  = next((c for c in df.columns if "AGENCI" in c), None)
    # Detalhamento: coluna cujo nome contém "DETAL" ou "HISTOR" (fallback)
    col_det = next((c for c in df.columns if "DETAL" in c), None)
    if col_det is None:
    ...
    if col_det:
        df["_DETALHAMENTO"] = df[col_det].fillna("").astype(str).str.strip().str.upper()
    else:
        df["_DETALHAMENTO"] = ""
...
def _bb_gerar_excel(df):
    """
    Gera Excel com todas as colunas originais do BB intactas,
    acrescentando UF e MUNICIPIO (colunas internas _* são omitidas).
    """
    import io, openpyxl as _opx
    ...
    # Colunas originais (sem as internas _*)
    orig_cols = [c for c in df.columns if not c.startswith("_")
                 and c not in ("UF","MUNICIPIO","TITULO","STATUS")]
    final_cols = orig_cols + ["UF","MUNICIPIO","TITULO","STATUS"]

    # Cores de destaque para colunas adicionadas
    ADDED_BG = {"UF":"7030A0","MUNICIPIO":"7030A0","TITULO":"FFFFFF","STATUS":"FFFFFF"}
    ...
    for ci, col in enumerate(final_cols, 1):
        ...
        # Largura automática com mínimo razoável
        width = 28 if col in ("MUNICIPIO","DETALHAMENTO","HISTORICO") else (5 if col == "UF" else 14)
        ws.column_dimensions[get_column_letter(ci)].width = width

    ws.row_dimensions[1].height = 28
    ALT_FILL = PatternFill("solid", fgColor="EBF3FB")

    # Identificar colunas de valor (numéricas) e agência para formatar corretamente
    col_valor_idx  = [i for i,c in enumerate(final_cols,1) if "VALOR" in c or "VLR" in c]
    col_agencia_idx= [i for i,c in enumerate(final_cols,1) if "AGENCI" in c]
    fmt_brl  = "#,##0.00"
    ...
    for ri, (_, row) in enumerate(df.iterrows(), 2):
        ...
        for ci, col in enumerate(final_cols, 1):
            ...
            # Limpar nulos
            if raw is None or str(raw).strip().upper() in ("NAN","NONE",""):
                val = None
            else:
                val = raw
            ...
            if ci in col_valor_idx and isinstance(val, (int, float)):
...
1. 📍 **Nome de município no texto** → busca direta na tabela de agências
2. 🔍 **CNPJ no detalhamento** → consulta BrasilAPI para obter município e UF
...
4. ⚠️ **Sem match** → marca como `VERIFICAR`
    ...
    for i, (idx, row) in enumerate(df.iterrows()):
        det = row["_DETALHAMENTO"]
        ag  = row["_AGENCIA_ORIG"]
        mun, uf = "", ""

        # 1) Nome/texto no detalhamento → tabela agências (prioritário)
        mun, uf = _bb_buscar_municipio(det, municipio_list)
        if mun:
            n_texto += 1

        # 2) CNPJ no detalhamento → BrasilAPI
        if not mun:
        ...
        # 3) Agência de origem → tabela agências
        if not mun:
    ...
    COLS_PREV = orig_cols[:5] + ["UF","MUNICIPIO"]
    cols_ok   = [c for c in COLS_PREV if c in df.columns and not c.startswith("_")]
